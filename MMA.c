//---------------------------------------------------------------------------| MMA von Dietmar SCHRAUSSER 2010              
//
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
{	
	...
	inStream = fopen( argv[2], "r" ); do { fscanf(inStream, "%s", tmp); nk_2++;} while (feof (inStream) == 0);   fclose( inStream  );
...
void _kopf() 
{
	printf("\nMMA by Dietmar Schrausser\n");
	printf("compiled on %s @ %s\n", __DATE__, __TIME__);
	printf("computing MMA:\n"); 
}
